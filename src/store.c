...
/* {{{ statics */
static void pthreads_store_create(pthreads_storage *storage, zval *pzval,
		zend_bool complex TSRMLS_DC);
static int pthreads_store_convert(pthreads_storage *storage,
		zval *pzval TSRMLS_DC);
static int pthreads_store_tostring(zval *pzval, char **pstring, size_t *slength,
		zend_bool complex TSRMLS_DC);
static int pthreads_store_tozval(zval *pzval, char *pstring,
		size_t slength TSRMLS_DC);
...
pthreads_store pthreads_store_alloc(TSRMLS_D) {
	...
	if (store) {
		if (zend_hash_init(&store->table, 8, NULL,
...
zend_bool pthreads_store_lock(zval *this_ptr TSRMLS_DC) {
	...
	if (pobject) {
		return pthreads_lock_acquire(pobject->store->lock,
...
zend_bool pthreads_store_unlock(zval *this_ptr TSRMLS_DC) {
	PTHREAD pobject = PTHREADS_FETCH_FROM(getThis());
	if (pobject) {
		return pthreads_lock_release(pobject->store->lock,
				pobject->hold TSRMLS_CC
				);
	} else
		return 0;
...
zend_bool pthreads_store_isset(pthreads_store store, char *key, int keyl,
		int has_set_exists TSRMLS_DC) {
	zend_bool locked = 0, isset = 0;
	if (store) {
		if (pthreads_lock_acquire(store->lock, &locked TSRMLS_CC)) {
			isset = zend_hash_exists(&store->table, key, keyl + 1);
			pthreads_lock_release(store->lock, locked TSRMLS_CC);
		}
	}
	return isset;
...
int pthreads_store_read(pthreads_store store, char *key, int keyl,
		zval **read TSRMLS_DC) {
	zend_bool locked = 0;
	int result = FAILURE;
	if (store) {
		...
		if (result == SUCCESS) {
			...
			FREE_ZVAL(*read);
			*read = EG(uninitialized_zval_ptr);
			Z_ADDREF_P(*read);
		}
	}
	return SUCCESS;
...
int pthreads_store_write(pthreads_store store, char *key, int keyl,
		zval **write TSRMLS_DC) {
	int result = FAILURE;
	zend_bool locked;
	...
	if (store) {
		...
		pthreads_storage storage;
		pthreads_storage *old_storage = NULL;

		pthreads_store_create(&storage, *write, 1 TSRMLS_CC);
...
/* {{{ seperate a zval using internals */
int pthreads_store_separate(zval * pzval, zval **separated, zend_bool allocate,
		zend_bool complex TSRMLS_DC) {
	int result = FAILURE;
	pthreads_storage storage;
...
int pthreads_store_count(zval *object, long *count TSRMLS_DC) {
	...
	if (pthreads) {
		if (pthreads_store_lock(object TSRMLS_CC)) {
			(*count) = zend_hash_num_elements(&pthreads->store->table);
...
int pthreads_store_shift(zval *object, zval **member TSRMLS_DC) {
	...
	if (pthreads) {
		if (pthreads_store_lock(object TSRMLS_CC)) {
			...
			if (zend_hash_get_current_data_ex(table, (void**) &storage,
					&position) == SUCCESS) {
				char *key;
				uint klen;
				ulong idx;
				...
				zend_hash_del_key_or_index(table, key, klen, idx,
						zend_hash_get_current_key_ex(table, &key, &klen, &idx,
								0, &position) == HASH_KEY_IS_STRING ?
...
/* {{{ store chunk */
int pthreads_store_chunk(zval *object, long size, zend_bool preserve,
		zval **chunk TSRMLS_DC) {
	PTHREAD pthreads = PTHREADS_FETCH_FROM(object);
	...
	if (pthreads) {
		if (pthreads_store_lock(object TSRMLS_CC)) {
			...
			while ((zend_hash_num_elements(Z_ARRVAL_PP(chunk)) < size)
					&& (zend_hash_get_current_data_ex(table, (void**) &storage,
							&position) == SUCCESS)) {
				char *key;
				uint klen;
				ulong idx = 0L;
				...
				zend_hash_del_key_or_index(table, key, klen, idx,
						(ktype = zend_hash_get_current_key_ex(table, &key,
								&klen, &idx, 0, &position))
								== HASH_KEY_IS_STRING ?
						HASH_DEL_KEY :
														HASH_DEL_INDEX);
				...
				if (!preserve) {
					zend_hash_next_index_insert(Z_ARRVAL_PP(chunk),
							(void** )&member, sizeof(zval), NULL);
				} else {
					if (ktype == HASH_KEY_IS_STRING) {
						zend_hash_update(Z_ARRVAL_PP(chunk), key, klen,
								(void** ) &member, sizeof(zval), NULL);
					} else
						zend_hash_index_update(Z_ARRVAL_PP(chunk), idx,
								(void** )&member, sizeof(zval), NULL);
				}
	...
	if (store) {
		if (pthreads_lock_acquire(store->lock, &locked TSRMLS_CC)) {
			...
			for (zend_hash_internal_pointer_reset_ex(stored, &position);
					zend_hash_get_current_data_ex(stored, (void**) &storage,
							&position) == SUCCESS;
				...
				if (zend_hash_get_current_key_ex(stored, &name, &nlength, &idx,
						0, &position) == HASH_KEY_IS_STRING) {
					if (name[0] != '$') { /* do not copy internal counter */
						char *rename = estrndup(name, nlength);
						{
							...
							if (pthreads_store_convert(storage, pzval TSRMLS_CC)
									!= SUCCESS) {
								ZVAL_NULL(pzval);
							}

							zend_hash_update(hash, rename, nlength, &pzval,
									sizeof(zval), NULL);
						}
						efree(rename);
					}
...
static void pthreads_store_create(pthreads_storage *storage, zval *unstore,
		zend_bool complex TSRMLS_DC) {
	...
	if (unstore) {
		...
		case IS_STRING: {
			if ((storage->length = Z_STRLEN_P(unstore))) {
				...
				memcpy(storage->data, (const void*) Z_STRVAL_P(unstore),
						storage->length + 1);
			}
		}
			break;
		...
		case IS_OBJECT:
		case IS_ARRAY: {
			if (pthreads_store_tostring(unstore, (char**) &storage->data,
					&storage->length, complex TSRMLS_CC) == SUCCESS) {
...
static int pthreads_store_convert(pthreads_storage *storage,
	...
	if (storage) {
		...
		case IS_NULL:
			...
			break;
		case IS_BOOL:
			ZVAL_BOOL(pzval, storage->simple.lval);
			break;
		case IS_LONG:
			ZVAL_LONG(pzval, storage->simple.lval)
			;
			break;
		case IS_DOUBLE:
			ZVAL_DOUBLE(pzval, storage->simple.dval)
			;
			break;
		case IS_RESOURCE: {
			...
			if (EG(scope) != resource->scope) {
				...
				if (resource->ls
						!= TSRMLS_C &&zend_hash_index_find(
								&PTHREADS_EG(resource->ls, regular_list),
								storage->simple.lval, (void**) &original)
						== SUCCESS) {
					...
					int existed = 0;
					{
						...
						for (zend_hash_internal_pointer_reset_ex(
								&EG(regular_list), &position);
								zend_hash_get_current_data_ex(&EG(regular_list),
										(void**) &search, &position) == SUCCESS;
								zend_hash_move_forward_ex(&EG(regular_list),
										&position)) {
					...
					if (!found && EG(This)) {
						...
						zend_rsrc_list_entry create;
						{
							...
							create.refcount = 1;
							created = zend_hash_next_free_element(
									&EG(regular_list));
							...
							if (zend_hash_index_update(
								...
								pthreads_resources_keep(object->resources,
										&create, resource TSRMLS_CC
										);
							} else
								ZVAL_NULL(pzval);
						}
		...
		case IS_ARRAY:
		case IS_OBJECT: {
			result = pthreads_store_tozval(pzval, (char*) storage->data,
...
static int pthreads_store_tostring(zval *pzval, char **pstring, size_t *slength,
	...
	if (pzval && (Z_TYPE_P(pzval) != IS_NULL)
			&& (Z_TYPE_P(pzval) != IS_OBJECT
					|| pthreads_store_validate_object(&pzval TSRMLS_CC))) {
			...
			*slength = psmart->len;
			if (psmart->len) {
				*pstring = malloc(psmart->len + 1);
				if (*pstring) {
					memcpy((char*) *pstring, (const void*) psmart->c,
							psmart->len);
					result = SUCCESS;
				}
			} else
	...
	if (pstring) {
		...
		if (pointer) {
			 ...
			 */
			{
				...
				if (!php_var_unserialize(&pzval, &pointer, pointer + slength,
						&vars TSRMLS_CC)) {
					result = FAILURE;
					zval_dtor(pzval);
				}
...
int pthreads_store_merge(zval *destination, zval *from,
		zend_bool overwrite TSRMLS_DC) {
	...
	case IS_OBJECT: {
		...
		if (IS_PTHREADS_OBJECT(from)) {
			...
			PTHREAD pobjects[2] = { PTHREADS_FETCH_FROM(destination),
					PTHREADS_FETCH_FROM(from) };

			/* acquire destination lock */
			if (pthreads_lock_acquire(pobjects[0]->store->lock,
					&locked[0] TSRMLS_CC)) {

				/* acquire other lock */
				if (pthreads_lock_acquire(pobjects[1]->store->lock,
						&locked[1] TSRMLS_CC)) {
					...
					HashPosition position;
					pthreads_storage *storage;
					HashTable *tables[2] = { &pobjects[0]->store->table,
							&pobjects[1]->store->table };

					for (zend_hash_internal_pointer_reset_ex(tables[1],
							&position);
							zend_hash_get_current_data_ex(tables[1],
									(void**) &storage, &position) == SUCCESS;
						...
						if (zend_hash_get_current_key_ex(tables[1], &key, &klen,
								&idx, 0, &position) == HASH_KEY_IS_STRING) {

							/* skip if not overwriting where the
							 entry exists in destination table */
							if (!overwrite
									&& zend_hash_exists(tables[0], key, klen)) {
								continue;
							}
							...
							if (storage->type != IS_RESOURCE) {
								...
								case IS_BOOL:
								case IS_LONG: {
									copy.simple.lval = storage->simple.lval;
								}
									break;

								case IS_DOUBLE: {
									copy.simple.dval = storage->simple.dval;
								}
									break;
								...
								zend_hash_update(tables[0], key, klen,
										(void** ) &copy,
										sizeof(pthreads_storage), NULL);
							}
						}
					}

					pthreads_lock_release(pobjects[1]->store->lock,
							locked[1] TSRMLS_CC);
				}

				pthreads_lock_release(pobjects[0]->store->lock,
						locked[0] TSRMLS_CC);
	...
	default: {
		...
		if (pthreads_lock_acquire(pobject->store->lock, &locked TSRMLS_CC)) {
			...
			zend_uint index = 0;
			HashTable *table =
					(Z_TYPE_P(from) == IS_ARRAY) ?
							Z_ARRVAL_P(from) : Z_OBJPROP_P(from);

			for (zend_hash_internal_pointer_reset_ex(table, &position);
					zend_hash_get_current_data_ex(table, (void**) &pzval,
							&position) == SUCCESS;
					zend_hash_move_forward_ex(table, &position)) {
				char *key;
				zend_uint klen;
				...
				switch (zend_hash_get_current_key_ex(table, &key, &klen, &idx,
						0, &position)) {
				case HASH_KEY_IS_STRING: {
					...
					pthreads_store_write(pobject->store, key, klen - 1,
							pzval TSRMLS_CC);
				}
					break;
				...
				case HASH_KEY_IS_LONG: {
					...
					if (!overwrite
							&& zend_hash_exists(table, Z_STRVAL(zkey),
									Z_STRLEN(zkey))) {
						zval_dtor(&zkey);
						goto next;
					}

					pthreads_store_write(pobject->store, Z_STRVAL(zkey),
							Z_STRLEN(zkey), pzval TSRMLS_CC);
				...
				default: {
					zend_error(E_WARNING,
							"pthreads detected an unsupported key type for merging, ignoring data at %lu",
							(long) index);
				}
				}
				next: index++;
...
static int pthreads_store_remove_complex_recursive(zval **pzval TSRMLS_DC) {
	...
	case IS_OBJECT:
		...
		if (thash) {
			zend_hash_apply(thash,
					(apply_func_t) pthreads_store_remove_complex TSRMLS_CC);
		}
